import { getModelForClass } from "@typegoose/typegoose";
import { ObjectType } from "type-graphql";
import { Groupe, GroupeCustomQuery, GroupeSearchQuery } from "./_groupe.type";
import { PaginationMedia } from "../../utils";
import { Media } from "../../utils/_media.base";

@ObjectType()
export class GroupePaginationMedia extends PaginationMedia<Groupe>(Groupe) { }

@ObjectType()
export class GroupeMedia extends Media<Groupe>(Groupe, GroupeSearchQuery.queryParse, GroupeSearchQuery.dynamicPopulate) { }

export const GroupeModel = getModelForClass<typeof GroupeMedia, GroupeCustomQuery>(GroupeMedia, { schemaOptions: { toJSON: { virtuals: true } } });

@ObjectType()
export class GroupeMediaPagination extends PaginationMedia(GroupeMedia) { }
