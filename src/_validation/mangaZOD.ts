...
import { MediaGenresArray, MediaParentLabelArray, MediaSourceArray, MediaStatusArray, dateTimeToZod, dateToZod } from "../_utils/mediaUtil";
import { z } from "zod";
import { Add_Character_ZOD } from "./characterZOD";
import { Add_Company_ZOD } from "./companyZOD";
import { Add_Groupe_ZOD } from "./groupeZOD";
import { Create_Link_ZOD, MediaDate_validation, MediaImage_validation, MediaTitle_validation } from "./media";
...
export const MangaDataToZOD = (data: IManga): Partial<ICreate_Manga_ZOD> => {
  ...
  let toZOD: Partial<ICreate_Manga_ZOD> = {
    groupe: data.groupe,
    parent: data.parent,
    source: data.source,
  ...
  let safeParse = Create_Manga_ZOD.safeParse(toZOD);
