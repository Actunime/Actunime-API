...
import { ClientSession, Mongoose, Query } from 'mongoose';
...
type MethodOption<J extends boolean, E extends boolean> = {
  json?: J;
...
export class Anime extends ClassUtilSession implements IAnime {
  ...
  async getDBDiff() {
    const original = await Anime.get(this.id, {
      cache: false,
      nullThrowErr: true,
    });
