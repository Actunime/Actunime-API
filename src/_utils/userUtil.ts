...
export const UserRolesLevel = Object
  .fromEntries((Object.keys(UserRolesObj) as IUserRoles[])
    .map((key) => [key, UserRolesObj[key].level])) as Record<IUserRoles, number>;

export const UserRolesLabel = Object.fromEntries((Object.keys(UserRolesObj) as IUserRoles[])
  .map((key) => [key, UserRolesObj[key].label])) as Record<IUserRoles, string>;

export const UserRolesArray = Object.keys(UserRolesObj) as IUserRoles[] & [string, ...string[]]
export const UserRolesSelection = UserRolesArray.map((key) => ({ label: UserRolesObj[key].label, value: key }))

export const userHasRole = (userRoles: IUserRoles[], roles: IUserRoles | IUserRoles[]) => {
...
export const userPermissionLevel = (userRoles: IUserRoles[]) => userRoles.reduce((acc, role) => acc | getRoleLevel(role), 0)

export const userPermissionIsHigherThan = (userRoles: IUserRoles[], roles: IUserRoles | IUserRoles[]) => {
  if (Array.isArray(roles)) {
