...
export function isEqualObject([obj1, obj2]: [any, any]): boolean {
  ...
  function EqualKey([obj1, obj2]: [object, object]): {
    ...
    const keysEqual: { [key: string]: boolean } = {};
  ...
  return (
    (Object.keys(equalKeys).length && Object.keys(equalKeys).every((key) => equalKeys[key])) ||
    false
  );
}
...
export function isEqualArray([array1, array2]: [any, any]) {
  ...
  if (equalsValueFound.length === array1.length && equalsValueFound.length === array2.length) {
...
export function changedObjKeys<T extends { [key: string]: any }>([oldOjb, newObj]: [T, any]) {
  ...
  const oldLenght = Object.keys(oldOjb || {}).length;
  const newLenght = Object.keys(newObj || {}).length;
  ...
  for (const key in obj) {
    if (Array.isArray(oldOjb?.[key]) || Array.isArray(newObj?.[key])) {
      ...
      const changes = changedObjKeys([oldOjb[key], newObj[key]]);
      if (Object.keys(changes.changes).length)
        changes[key as keyof typeof changes] = changes.changes;

      if (Object.keys(changes.before).length) before[key] = changes.before;
  ...
  // Nouveaux champs
  // for (const key in cleanObject(newObj || object)) {
...
export function ObjCheckChanged(prevData: any, newData: any, ignoreKeys: string[] = []) {
  ...
  const keys = getKeys.filter(
    (ele, pos) => getKeys.indexOf(ele) === pos && !ignoreKeys.includes(ele)
  );
  ...
  for (const key of keys) {
    ...
    if (typeofPrevData === 'object' || typeofNewData === 'object') {
      ...
      const trimedPrevDataValue = prevDataValue && prevDataValue.trim();
      const trimedNewDataValue = newDataValue?.trim();
...
export function ArrayCheckChanged(prevData: Array<any>, newData: Array<any>) {
  ...
  for (let i = 0; i < prevData.length; i++) {
    ...
    if (typeof element !== 'object') {
      if (!newData.includes(element)) removed.push(element);
    } else {
  ...
  for (let i = 0; i < newData.length; i++) {
    ...
    if (typeof element !== 'object') {
      if (!prevData.includes(element)) added.push(element);
    } else {
      throw new Error('ArrayCheckChanged ne supporte pas les tableaux de type object');
    }
  }
