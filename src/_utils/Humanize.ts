...
export function HumanizeDate(date?: string | Date, noTime: boolean = false) {
  ...
  try {
    ...
    let localTime = noTime ? formatedDate.toLocaleDateString() : formatedDate.toLocaleTimeString();
    let local = noTime ? formatedDate.toLocaleDateString() : formatedDate.toLocaleString();
