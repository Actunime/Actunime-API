        ...
        async (user) => {
            try {
                ...
                pagination.addSearchQuery([
                    ...(query?.status ? [Array.isArray(query.status) ? { status: { $in: query.status } } : { status: query.status }] : []),
                    ...(query?.actionLabel ? [{ "actions.label": query.actionLabel, }] : []),
                    ...(query?.actionUser ? [{ "actions.user.id": query.actionUser }] : []),
                    ...(query?.type) ? [Array.isArray(query.type) ? { type: { $in: query.type } } : { type: query.type }] : [],
                    ...(query?.author ? [{ "author.id": query.author }] : []),
                    ...(query?.target ? [{ "target.id": query.target }] : []),
                    ...(query?.targetPath ? [Array.isArray(query.targetPath) ? { targetPath: { $in: query.targetPath } } : { targetPath: query.targetPath }] : []),
                ...
                if (data.with?.author)
                    await UpdateModel.populate(response.results, "author.data");

                if (data.with?.target)
                    await UpdateModel.populate(response.results, "target.data");

                if (data.with?.ref)
                    await UpdateModel.populate(response.results, "ref.data");

                if (data.with?.actions)
                    await UpdateModel.populate(response.results, "actions.user.data");
