...
export async function Get(req: FastifyRequest<{ Params: { id: string } }>) {
  ...
  if (data.with?.author) await findUpdate.populate({ path: 'author.data' });

  if (data.with?.target) await findUpdate.populate({ path: 'target.data' });

  if (data.with?.ref) await findUpdate.populate({ path: 'ref.data' });

  if (data.with?.actions) await findUpdate.populate({ path: 'actions.user.data' });
