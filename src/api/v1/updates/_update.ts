...
export async function Update(
  req: FastifyRequest<{ Params: { id: string }; Body: IUpdate_Action_ZOD }>
) {
  return RestrictedAPIRoute(
    'MODERATOR',
    ...
    async (user) => {
      ...
      try {
        ...
        const targetSaveDB =
          TargetPathSaveDB[findUpdate.targetPath as keyof typeof TargetPathSaveDB];
        ...
        switch (label) {
          case 'IN_PROGRESS':
            ...
            await findUpdate.updateOne(
              { status: 'IN_PROGRESS', $push: { actions: action } },
              { runValidators: true }
            );
            break;
          ...
          case 'CHANGE': {
            if (findUpdate.status !== 'PENDING' && findUpdate.status !== 'IN_PROGRESS')
              throw new ErrorHandled(
                'Le statut de la modification ne permet pas une modification de celle-ci'
              );

            if (!Object.keys(changes || {}).length)
              throw new ErrorHandled("Aucune modification n'a été détectée");

            const { changes: changedValues } = ObjCheckChanged(
              findUpdate.beforeChanges,
              { ...findUpdate.changes, ...changes },
              ['id', '_id']
            );

            if (!changedValues) throw new ErrorHandled("Aucune modification n'a été détectée");

            await findUpdate.updateOne(
              { changes: changedValues, $push: { actions: action } },
              { runValidators: true }
            );
            break;
          }
          case 'REJECT':
            if (
              findUpdate.status === 'ACCEPTED' ||
              findUpdate.status === 'REVERTED' ||
              findUpdate.status === 'REJECTED'
            )
              throw new ErrorHandled(
                'Le statut de la modification ne permet pas un rejet de celle-ci'
              );

            await findUpdate.updateOne(
              { status: 'REJECTED', $push: { actions: action } },
              { runValidators: true }
            );
            break;
          ...
          case 'ACCEPT': {
            if (findUpdate.status === 'ACCEPTED' || findUpdate.status === 'REJECTED')
              throw new ErrorHandled(
                "Le statut de la modification ne permet pas l'acceptation de celle-ci"
              );

            if (!findUpdate.target) throw new ErrorHandled("Le target n'existe pas");
            ...
            if (!targetPreCreated) throw new ErrorHandled("Le target n'existe pas");

            const { changes: getChanges } = ObjCheckChanged(
              targetPreCreated.toJSON(),
              hydratedChanges.toJSON(),
              ['id', '_id']
            );

            if (getChanges) console.log('Modifications détectées', getChanges);
            ...
            await findUpdate.updateOne(
              { status: 'ACCEPTED', $push: { actions: action } },
              { runValidators: true }
            );
          ...
          case 'REVERT':
            if (findUpdate.status === 'ACCEPTED') {
              ...
              throw new ErrorHandled(
                "Vous ne pouvez pas annulé une modification qui n'a pas été accepté."
              );
            }

            await findUpdate.updateOne(
              { status: 'REVERTED', $push: { actions: action } },
              { runValidators: true }
            );
            break;
