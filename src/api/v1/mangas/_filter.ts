...
export async function Filter(req: FastifyRequest<{ Querystring: { pagination?: string } }>) {
    try {
        ...
        if (query?.name)
            pagination.searchByName(query.name, "name.full");
        ...
        if (data.with?.groupe)
            await MangaModel.populate(response.results, 'groupe.data');
        if (data.with?.parent)
            await MangaModel.populate(response.results, 'parent.data');
        if (data.with?.source)
            await MangaModel.populate(response.results, 'source.data');
        if (data.with?.staffs)
            await MangaModel.populate(response.results, 'staffs.data');
        if (data.with?.companys)
            await MangaModel.populate(response.results, 'companys.data');
        if (data.with?.characters)
            await MangaModel.populate(response.results, 'characters.data');
