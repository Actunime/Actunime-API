...
export type IPersonDoc = Document<any, object, IPerson> &
  IPerson &
...
export async function CheckPersonUnique(first: string, last: string, notId?: string) {
  ...
  if (notId && findPersonByName?.id === notId) return;

  if (findPersonByName) throw new ErrorHandled('Une personne est déjà enregistrer avec ce nom.');
}
...
export async function PersonSaveDB(props: Partial<ICreate_Person_ZOD>, user: IUser) {
  ...
  const newPerson = { ...addPerson } as any as IPerson;
    ...
    create: async (options?: {
      ...
      try {
        ...
        throw new ErrorHandled(
          "Une erreur s'est produite lors de la sauvegarde de la personnalité"
        );
      }
    },
    update: async (
      ...
      try {
        ...
        if (wasUpdated)
          await findPerson
            .updateOne({ $set: findPerson.toJSON() }, { runValidators: true })
            .catch(() => {
              console.error(
                `La modification effectué sur la personnalité ${findPerson.id} n'a pas pu être annulé.`
              );
            });
        throw new ErrorHandled(
          "Une erreur s'est produite lors de la sauvegarde de la personnalité"
