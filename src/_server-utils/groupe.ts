...
import {
  GroupeDataToZOD,
  type IAdd_Groupe_ZOD,
  type ICreate_Groupe_ZOD
} from '../_validation/groupeZOD';
...
type IGroupeDoc = Document<any, object, IGroupe> &
  IGroupe &
...
export async function CheckGroupeUnique(name: string, notId?: string) {
  ...
  if (notId && findGroupeByName?.id === notId) return;

  if (findGroupeByName) throw new ErrorHandled('Un groupe est déjà enregistrer avec ce nom.');
}

export async function GroupeSaveDB(props: Partial<ICreate_Groupe_ZOD>, user: IUser) {
  const mediasToSave: IMediaDocTypeToSave[] = [];

  const { ...addGroupe } = props;
  const newGroupe = { ...addGroupe } as any as IGroupe;
  ...
  return {
    ...
    update: async (
      ...
      if (objLevel1?.name) {
        await CheckGroupeUnique(objLevel1.name, findGroupe.id);
      }
      let update: any,
        activity,
        wasUpdated = false;
      try {
        ...
        if (wasUpdated)
          await findGroupe
            .updateOne({ $set: findGroupe.toJSON() }, { runValidators: true })
            .catch(() => {
              console.error(
                `La modification effectué sur le groupe ${findGroupe.id} n'a pas pu être annulé.`
              );
            });
        throw new ErrorHandled("Une erreur s'est produite lors de la sauvegarde de le groupe");
      }
