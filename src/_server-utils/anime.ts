...
// type IAnimeDoc = Document<unknown, {}, IAnime> & IAnime & Required<{ _id: Schema.Types.ObjectId; }>
...
export async function CheckAnimeUnique(name: string, notId?: string) {
  ...
  if (notId && findAnimeByName?.id === notId)
    return;

  if (findAnimeByName)
    throw new ErrorHandled("Un anime est déjà enregistrer avec ce nom.");
}
...
export async function AnimeSaveDB(props: Partial<ICreate_Anime_ZOD>, user: IUser) {
  ...
  const newAnime = { ...addAnime } as unknown as IAnime;
  ...
  return {
    ...
    update: async (id: string, options?: {
      ...
      try {
        ...
        if (wasUpdated)
          await findAnime.updateOne({ $set: findAnime.toJSON() }, { runValidators: true })
            .catch(() => {
              console.error(`La modification effectué sur l'anime ${findAnime.id} n'a pas pu être annulé.`)
            });
