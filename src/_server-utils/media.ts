...
type AnyDocType = Document<any, object, any> & Required<{ _id: Schema.Types.ObjectId }>;
type AnyModelType = Model<any, object, object, object, AnyDocType>;
...
export async function BulkCreateMedias(options: {
      ...
      for await (const { doc, path } of savedMedias) {
        ...
        if (options?.withActivity) {
          const activity = await CreateActivity(
            ...
            ('CREATE_' + path.toUpperCase()) as IActivityAction,
            {
              author: { id: options?.authorId },
              target: { id: doc.id },
              targetPath: path
...
export async function BulkRequestMedias(options: {
  medias: IMediaDocTypeToSave[];
  authorId: string;
  updateRef?: string;
  actions: IUpdateActionList[];
