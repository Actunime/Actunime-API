...
export class MediaPagination<T = any> {
    ...
    public setSort(sort: Record<string, "ASC" | "DESC">) {
        ...
        var formated: Record<string, 1 | -1> = Object.fromEntries(keys.map((key) => [key, sort[key] === "ASC" ? 1 : -1]));
        this.sort = formated;
    }
    ...
    public async getResults() {
        ...
        var resData: IPaginationResponse<T>[] = await this.model.aggregate(aggregation).exec();
