...
type ICompanyDoc = Document<any, object, ICompany> &
  ICompany &
...
export async function CheckCompanyUnique(name: string, notId?: string) {
  ...
  if (notId && findCompanyByName?.id === notId) return;

  if (findCompanyByName) throw new ErrorHandled('Une société est déjà enregistrer avec ce nom.');
}
...
export async function CompanySaveDB(props: Partial<ICreate_Company_ZOD>, user: IUser) {
  ...
  const newCompany = { ...addCompany } as any as ICompany;
  ...
  return {
    ...
    update: async (
      ...
      try {
        ...
        if (wasUpdated)
          await findCompany
            .updateOne({ $set: findCompany.toJSON() }, { runValidators: true })
            .catch(() => {
              console.error(
                `La modification effectué sur la société ${findCompany.id} n'a pas pu être annulé.`
              );
            });
        throw new ErrorHandled("Une erreur s'est produite lors de la sauvegarde de la société");
      }
