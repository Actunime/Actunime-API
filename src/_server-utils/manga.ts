...
export async function CheckMangaUnique(name: string, notId?: string) {
  ...
  if (notId && findMangaByName?.id === notId) return;

  if (findMangaByName) throw new ErrorHandled('Un manga est déjà enregistrer avec ce nom.');
}
...
export async function MangaSaveDB(props: Partial<ICreate_Manga_ZOD>, user: IUser) {
  ...
  const newManga = { ...addManga } as any as IManga;
  ...
  return {
    ...
    update: async (
      ...
      try {
        ...
        if (wasUpdated)
          await findManga
            .updateOne({ $set: findManga.toJSON() }, { runValidators: true })
            .catch(() => {
              console.error(
                `La modification effectué sur le manga ${findManga.id} n'a pas pu être annulé.`
              );
            });
        throw new ErrorHandled("Une erreur s'est produite lors de la sauvegarde du manga");
      }
