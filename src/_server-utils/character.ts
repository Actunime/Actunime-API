...
type ICharacterDoc = Document<any, object, ICharacter> &
  ICharacter &
...
export async function CharacterSaveDB(props: Partial<ICreate_Character_ZOD>, user: IUser) {
  ...
  const newCharacter = { ...addCharacter } as any as ICharacter;
  ...
  return {
    ...
    update: async (
      ...
      try {
        ...
        if (wasUpdated)
          await findCharacter
            .updateOne({ $set: findCharacter.toJSON() }, { runValidators: true })
            .catch(() => {
              console.error(
                `La modification effectué sur le peronnsage ${findCharacter.id} n'a pas pu être annulé.`
              );
            });
        throw new ErrorHandled("Une erreur s'est produite lors de la sauvegarde de le peronnsage");
      }
