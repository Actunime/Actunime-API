...
type ITrackDoc = Document<unknown, {}, ITrack> & ITrack & Required<{ _id: Schema.Types.ObjectId; }>
  ...
  try {
    ...
    for await (const add of addMultiple) {
      if (add?.id) {
        ...
        await PersonAddToRelation(add.newTrack.artists, "artists",
...
export async function TrackSaveDB(props: Partial<ICreate_Track_ZOD>, user: IUser) {
  ...
  const newTrack = { ...addTrack } as unknown as ITrack;
  ...
  return {
    ...
    update: async (id: string, options?: {
      ...
      try {
        ...
        if (wasUpdated)
          await findTrack.updateOne({ $set: findTrack.toJSON() }, { runValidators: true })
            .catch(() => {
              console.error(`La modification effectué sur musique ${findTrack.id} n'a pas pu être annulé.`)
            });
        throw new ErrorHandled("Une erreur s'est produite lors de la sauvegarde de la musique");
      }
    },
