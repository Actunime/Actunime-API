import { ClientSession, Document, FilterQuery, Model, PipelineStage, Schema } from "mongoose";
...
class Pagination<T> {
    ...
    private sort: Partial<Record<keyof T, number>> | { createdAt: number } = { createdAt: -1 };
    ...
    constructor(model: IPaginationModelDoc<T>) { this.model = model; }
    ...
    public useFilter({ page, limit, strict, sort, query }: z.infer<typeof PaginationBody>, onlyVerified?: boolean) {
        if (page)
            this.setPage(page)
        if (limit)
            this.setLimit(page)
        ...
        if (onlyVerified !== undefined)
            this.setVerifiedOnly(onlyVerified);
    }
    ...
    private parseObj(obj: object, forSort: boolean = false) {
        ...
        for (const key of Object.keys(obj)) {
            ...
            if (Array.isArray(value)) {
                if (forSort) throw new APIError("La pagination n'accepte pas de tableau pour sort", "BAD_ENTRY");
                const hasObject = value.some(v => typeof v === 'object');
                if (hasObject) throw new APIError("La pagination n'accepte pas de tableau contenant un objet, revoir le schema de validation!", "BAD_ENTRY")
