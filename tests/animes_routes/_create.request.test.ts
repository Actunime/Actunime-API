...
const CREATE_REQUEST = async () => {
  ...
  await describe("Créer une demande de création d'un Anime", async () => {
    await it("Créer une demande d'anime sans être connecté", async () => {
    ...
    await it("Créer une demande d'anime avec des données incorrectes", async () => {
      const server = new Server(true);
      await server.start();
    ...
    await it("Créer une demande d'anime avec des données correctes", async () => {
      const server = new Server(true);
      await server.start();
    ...
    await it("Créer une demande d'anime avec des mauvaise permissions", async () => {
      const server = new Server(true);
      await server.start();
